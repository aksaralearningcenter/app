...
import { app, modal, closeModal } from '../helpers.js';
  ...
  export const render = {
    classes: function(list) {
      list = list || [];
      // Kolom “Sesi Jadwal” = jumlah sesi aktif di menu 🗓️ Jadwal, supaya
      // terlihat kelas mana yang jadwalnya belum diatur (0 sesi).
      const rows = list.map(c =>
        '<tr><td><b>' + esc(c.nama) + '</b></td><td>' + esc(c.guru || '-') + '</td><td>' + esc(c.jadwal || '-') + '</td>' +
        '<td>' + (c.jumlahSesi ? '<span class="badge b-ok">' + c.jumlahSesi + ' sesi</span>' : '<span class="badge b-warn">belum diatur</span>') + '</td>' +
        '<td>' + (c.kapasitas || '-') + '</td>' +
      ...
      $('page').innerHTML =
        ...
        '<div class="card"><div class="table-wrap"><table><thead><tr><th>Nama</th><th>Guru</th><th>Catatan Jadwal</th><th>Sesi Jadwal</th><th>Kapasitas</th><th>Biaya/Bulan</th><th>Status</th><th></th></tr></thead><tbody>' + rows + '</tbody></table></div>' +
        '<p style="font-size:.78rem; margin-top:10px;">Sesi belajar per pekan (hari, jam, mapel) diatur di menu <b>🗓️ Jadwal</b> — kolom “Catatan Jadwal” hanya ringkasan bebas.</p></div>';
    },
  };
  ...
  function openAddClass() {
    ...
    modal('➕ Tambah Kelas',
      ...
      '<div class="frow"><div class="fg"><label>Guru</label><select id="c-guru">' + guruOptions + '</select></div><div class="fg"><label>Catatan Jadwal</label><input id="c-jadwal" placeholder="Senin, Rabu"></div></div>' +
      '<p style="font-size:.75rem; margin:-6px 0 12px; color:var(--muted);">Sesi detail (jam &amp; mapel) diatur di menu <b>🗓️ Jadwal</b>.</p>' +
  ...
  function openEditClass(id) {
    ...
    modal('✏️ Edit Kelas',
      ...
      '<div class="frow"><div class="fg"><label>Guru</label><select id="c-guru">' + guruOptions + '</select></div><div class="fg"><label>Catatan Jadwal</label><input id="c-jadwal" value="' + esc(cls.jadwal || '') + '"></div></div>' +

// ==================== CHATBOT AKSARA ====================
// Chat teks polos untuk pengunjung umum (POST /public/chat); kunci API Gemini
// hanya ada di server. Lampiran foto/berkas & riwayat tersimpan hanya ada di
// halaman Asisten di dalam panel (khusus akun login). Widget tidak
...
(function () {
  ...
  let hariTerakhir = '';
  function tambah(kelas, html, waktu) {
    if (waktu) {
      let label = '';
      try {
        ...
        label = beda <= 0 ? 'Hari ini' : (beda === 1 ? 'Kemarin' :
          d.toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' }));
      } catch (_e) { /* abaikan */ }
      if (label && label !== hariTerakhir) {
        hariTerakhir = label;
        const p = document.createElement('div');
        p.className = 'chat-hari';
        p.innerHTML = '<span>' + teksAman(label) + '</span>';
        body.appendChild(p);
      }
    }
    ...
    d.innerHTML = html +
      (waktu ? '<time class="chat-waktu">' + teksAman(jamSingkat(waktu)) + '</time>' : '');
    body.appendChild(d);
  ...
  async function tanya(teks) {
    ...
    try {
      ...
      if (res && res.success && res.balasan) {
        tambah('bot', kaya(res.balasan), new Date().toISOString());
        ...
        tambah('err', kaya((res && res.message) || 'Maaf, asisten belum bisa menjawab. Coba lagi ya.'), new Date().toISOString());
      }
    } catch (_err) {
      tunggu.remove();
      tambah('err', kaya('Maaf, koneksi ke asisten gagal. Periksa jaringan Anda, atau hubungi tim kami lewat WhatsApp.'), new Date().toISOString());
    } finally {
      menunggu = false;
  ...
  function sambungkan() {
    terpasang = true;
    if (cfg.sapaan) tambah('bot', kaya(cfg.sapaan), new Date().toISOString());
